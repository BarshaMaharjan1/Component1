...
class HtmlProductWriter extends ShopProductWriter
{
    ...
    private function htmlBody()
    {
        ...
        foreach ($this->products as $product) {
         ...
         if($product instanceof GamesProduct) $gameproducts[] = $product;
        }
    ...
    private function generateGameTable($gameproducts){
        $contents = '';
        foreach ($gameproducts as $games) {
            $contents .= '<tr>
                  <td>'.$games->getFullName().'</td>'
                .'<td>'.$games->getTitle().'</td>'
                .'<td>'.$games->getPegiRating().'</td>'
                .'<td>'.$games->getPrice().'</td>'
                .'<td>'.'<a href="./index.php?delete='.$games->getId().'">X</a>'.'</td>
                </tr>';
        }
        return
            '
